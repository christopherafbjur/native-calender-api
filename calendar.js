...
class NativeCalendar {
  setup(titlePrefix, errorPrefix) {
    if (window.plugins && window.plugins.calendar) {
      ...
      console.warn(
        "this package is dependant on the calendar plugin please call setup After it has loaded"
      );
    }
  ...
  asyncDeleteEventById(id) {
    return this.asyncFindEventWithOptions(this.getDefaultBlankObject(), {
      id
    }).then(events => {
  ...
  asyncFindEventWithOptions(event, options) {
    return new P((resolve, reject) => {
      const { title, location, notes, startDate, endDate } = event;
      this.calendar.findEventWithOptions(
        title,
        location,
        notes,
        startDate,
  ...
  asyncDeleteEventFromNamedCalendar(event, calendarName) {
    ...
    return new P((resolve, reject) => {
      const { title, location, notes, startDate, endDate } = event;
      this.calendar.deleteEventFromNamedCalendar(
        title,
        location,
        notes,
        startDate,
